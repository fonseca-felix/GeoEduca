document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateReportSalas() {
    renderCustomSelectOptions('custom-select-report-sala-options', salas);
    setupCustomSelect('custom-select-report-sala', 'reportSalaSelect', () => window.loadReportData());
  }
  ...
  function populateSendQuizSalasSelect() {
    renderCustomSelectOptions('custom-select-send-quiz-sala-options', salas);
    setupCustomSelect('custom-select-send-quiz-sala', 'sendQuizSalaSelect');
  }
  ...
  function renderSendQuizAlunosList() {
    ...
    list.innerHTML = alunos.map(a => `
      <label class="send-quiz-student-item" style="display:flex; gap:0.6rem; align-items:center; margin-bottom:0.75rem; cursor:pointer;" data-nome="${a.nome.toLowerCase()}">
  ...
  window.filterSendQuizStudents = function() {
    const term = (document.getElementById('sendQuizSearchInput')?.value || '').toLowerCase();
    const items = document.querySelectorAll('#sendQuizAlunosList .send-quiz-student-item');
    items.forEach(el => {
